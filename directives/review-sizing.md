...
later launch can change it. The budgets are cumulative, not concurrency
slots: a launch frees nothing when it finishes, fails, is killed, is
interrupted, or is lost with the session, so a relaunch must fit what
the round has left. The cap also sets the cycle's derive seats, which
`opus-xhigh` and `fable-xhigh` share:
...
A cap fixed at `3` seats no derivation for the rest of the cycle. File
count, diff size, security subject matter, importance, and the ultracode
setting never raise the cap, and none of them is a derive.
...
never one agent per finding: `agent-scope:opus-medium` when the brief
hands it the claims and the lines, `agent-scope:opus-high` when it must
...
A derive seat needs `derive:` naming what the agent derives, one of the
kinds above. The one test: if an oracle outside the agent settles the
principal claim, the brief is `opus-high`, or `fable-high` where it
fails to split.
